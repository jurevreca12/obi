import obi_pkg::obi_a;
import obi_pkg::obi_r;
import obi_pkg::addr_map;


// OBI Manager-Router
module obi_manager_router #(
    parameter int ADDR_WIDTH = 32,
    parameter int DATA_WIDTH = 32,
    parameter int NBytes = DATA_WIDTH / 8,
    parameter int M_ROUTERS = 2,
    parameter bit [$clog2(M_ROUTERS)-1:0] MANAGER_ID = '0,
    parameter int S_ROUTERS = 8,
    parameter int ID_WIDTH = 32,
    parameter int NoMAPS = 1 // Number of mappings in the address map

)
(
    input   logic clk_i,
    input   logic rstn_i,

// OBI A channels, Switch -OBI-> Subordinate-Router
    output obi_pkg::obi_a [S_ROUTERS-1:0] obi_a_channels_o,
// OBI agnt signal, Switch <-OBI- Subordinate-Router
    input logic [S_ROUTERS-1:0] obi_agnt_array_i,

// OBI R channels, Switch <-OBI- Subordinate-Router
    input obi_pkg::obi_r [S_ROUTERS-1:0] obi_r_channels_i,
// OBI rready signal, Switch -OBI-> Subordinate-Router
    output logic [S_ROUTERS-1:0] obi_rready_array_o,
    
// Manager -OBI_A-> XBAR Manager-Router
    input logic                     obi_areq_i,
    input logic [ADDR_WIDTH-1:0]    obi_aadr_i,
    input logic                     obi_awe_i,
    input logic [NBytes-1:0]        obi_abe_i,
    input logic [DATA_WIDTH-1:0]    obi_awdata_i,
    input logic [ID_WIDTH-1:0]      obi_aid_i,
// Manager <-OBI_A- XBAR Manager-Router
    output logic                    obi_agnt_o,

// Manager -OBI_R-> XBAR Manager-Router
    input logic                     obi_rready_i,
// Manager <-OBI_R- XBAR Manager-Router
    output logic [DATA_WIDTH-1:0]   obi_rdata_o,
    output logic                    obi_rerr_o,
    output logic                    obi_rvalid_o,
    output logic [ID_WIDTH-1:0]     obi_rid_o,

// Address map used to map address space to Subordinates
    input obi_pkg::addr_map addr_map_i [NoMAPS]
);
// OBI Manager-Router -OBI_A-> Subordinate-Router
    obi_pkg::obi_a obi_a;

    assign obi_a.obi_areq   =   obi_areq_i;
    assign obi_a.obi_aadr   =   obi_aadr_i;
    assign obi_a.obi_awe    =   obi_awe_i;
    assign obi_a.obi_abe    =   obi_abe_i;
    assign obi_a.obi_awdata =   obi_awdata_i;
    assign obi_a.obi_aid    =   obi_aid_i;
    assign obi_a.obi_mid    =   MANAGER_ID;

// OBI Manager-Router <-OBI_A- Subordinate-Router
    obi_pkg::obi_r obi_r;

    assign obi_rdata_o      =   obi_r.obi_rdata;
    assign obi_rerr_o       =   obi_r.obi_rerr;
    assign obi_rvalid_o     =   obi_r.obi_rvalid;
    assign obi_rid_o        =   obi_r.obi_rid;

// Select signals used for switching by the OBI Switch
    logic [$clog2(S_ROUTERS)-1:0] obi_a_sel;
    logic [S_ROUTERS-1:0] obi_r_sel;

// Enable signal for setting next rid
    logic set_next;

// Array of rid signals used to compare with expected rid for switching
    logic [ID_WIDTH-1:0] rid_array[S_ROUTERS];

// R decoder
    // TODO propagate address_maps 
    logic address_map_err;
    /*
    soc_defines::addr_map [1:0]  address_maps;
    assign address_maps[0] = '{3'd0,32'h0000_0000,32'h4000_0000};
    assign address_maps[1] = '{3'd1,32'h4000_0000,32'h4000_0000};
    */

// OBI Manager-Router Switch
    obi_mr_switch #(
        S_ROUTERS,
        ID_WIDTH
    ) obi_mr_switch_inst (
        .obi_a_i(obi_a),
        .obi_r_o(obi_r),
        .obi_agnt_o(obi_agnt_o),
        .obi_rready_i(obi_rready_i),
        .obi_a_channels_o(obi_a_channels_o),
        .obi_r_channels_i(obi_r_channels_i),
        .obi_agnt_array_i(obi_agnt_array_i),
        .obi_rready_array_o(obi_rready_array_o),
        .obi_a_sel_i(obi_a_sel),
        .address_map_err_i(address_map_err),
        .obi_r_sel_i(obi_r_sel),
        .set_next_o(set_next),
        .rid_array_o(rid_array)
    );

// OBI A channel decoder 
    obi_a_decoder #(S_ROUTERS, ADDR_WIDTH, NoMAPS) obi_a_decoder_inst (
        .req_address_i(obi_aadr_i),
        .address_maps_i(addr_map_i),
        .config_ready_i(1'b1),
        .default_sel_en_i('0),
        .default_sel_i('0),
        .address_map_err_o(address_map_err),
        .obi_a_sel_o(obi_a_sel)
    );

// OBI R channel decoder
    obi_r_decoder #(
        S_ROUTERS,
        ID_WIDTH
    ) obi_r_decoder_inst(
        .clk_i(clk_i),
        .rstn_i(rstn_i),
        .set_next_i(set_next),
        .rid_array_i(rid_array),
        .obi_r_sel_o(obi_r_sel)
    );

endmodule
