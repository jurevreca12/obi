...
from test_mapped_slave import MappedSlaveTB

@forastero.sequence()
@forastero.requires("driver", MappedRequestInitiator)
async def mapped_slave_linear_read_seq(
    ctx: SeqContext,
...
@forastero.sequence()
@forastero.requires("driver", MappedRequestInitiator)
async def mapped_slave_random_read_seq(
    ctx: SeqContext,
    driver: SeqProxy[MappedRequestInitiator],
    tb: MappedSlaveTB,
    count: 1000,
...
@forastero.sequence()
@forastero.requires("driver", MappedRequestInitiator)
async def mapped_slave_linear_write_seq(
    ctx: SeqContext,
    driver: SeqProxy[MappedRequestInitiator],
    tb: MappedSlaveTB,
    addresses: list[int] | None = None,
...
@forastero.sequence()
@forastero.requires("driver", MappedRequestInitiator)
async def mapped_slave_random_write_seq(
    ctx: SeqContext,
    driver: SeqProxy[MappedRequestInitiator],
    tb: MappedSlaveTB,
    count: 1000,
